...
$pdf->Cell(0,10,'CERTIFICADO ESCOLAR',0,1,'C');
$pdf->Ln();
...
$pdf->SetFont('Arial','',14);
$pdf->Write(9,utf8_decode('. Se encuentra matriculado (a) en el grado '));
$pdf->SetFont('Arial','B',14);
$pdf->Write(9, strtoupper(utf8_decode(' '.$fila["level_grade"].' '.$fila["clasification"].'')));
$pdf->SetFont('Arial','',14);
$pdf->Write(9,utf8_decode(' en el año lectivo '. date("Y").'.'));
$pdf->Ln();
$pdf->SetFont('Arial','',14);
...
$pdf->Write(5,utf8_decode('La firma digital tiene validez'));
 ...
 $tema="Certificado de estudio";
  $mail = new PHPMailer(); 
    ...
    $mail->Subject = $tema;
    $mail->Body = 'Adjunto encontrarás el certificado escolar en un PDF .';
    $mail->addAddress($fila["email"]);
      //$mail->addStringEmbeddedImage("../images/QR.png", "");
 $mail->AddStringAttachment($pdfContent, 'certificado_estudio'.$nombres_completos.'.pdf');
