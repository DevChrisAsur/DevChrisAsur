...
if (isset($_GET['idPension'])) {
    ...
    // Consulta para obtener los datos del usuario según su ID
     $consulta="SELECT p.*, st.first_name,st.last_name, pt.id_parent, pt.first_name_parent, pt.last_name_parent, ph.phone, ps.* FROM pension p INNER JOIN student_pension ps ON p.id_pension = ps.id_pension INNER JOIN student st ON st.id_student = ps.id_student INNER JOIN student_parent sp ON st.id_student = sp.id_student INNER JOIN parent pt ON pt.id_parent = sp.id_parent INNER JOIN parent_phone ph ON ph.id_parent = pt.id_parent  WHERE p.id_pension=$idPension";
...
$token = 'EAAOC03X9yIUBO0SL1hDDL91ubuLm7klm1ONPykXGZCq7AchmtqFxQhjerMzCBu8uhzFT9sZAhotuZBeg9kAImpN0QjPnaZCDBRv2Xage5e83Vw0i0zc4TIMBqhd8mzIIbC1CXFGdMfI3N95lc0B2eBfd7lHTlhY0awTVwPAq0Vp8JpV5H5UDCE9wev6vnDqZBXCr3viZBaXaRRnZCXCvgZDZD';
//NUESTRO TELEFONO
